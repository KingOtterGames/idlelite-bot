...
const collectMessage = async (message) => {
  const outputBuffer = await svg2png({
    input: `<svg xmlns="http://www.w3.org/2000/svg" width="350" height="136" viewBox="0 0 350 136">
      <g id="template" transform="translate(0 0)">
        ...
        <text id="_usr_" data-name="${'name'}" transform="translate(10 10)" fill="#fff" font-size="20" font-family="SegoeUI, Segoe UI"><tspan x="0" y="0">Hello ${name}</tspan></text>
  ...
  message.channel.send({ files: [{ attachment: outputBuffer }] })
}
